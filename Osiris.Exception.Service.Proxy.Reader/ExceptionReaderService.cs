...
namespace Inmeta.Exception.Service.Proxy.Reader
{
    public partial class ExceptionReaderService : ServiceBase
    {
        ...
        private void SingleThreadedPollExceptionQueue()
        {
            ...
            //store them in TFS.
            response.ToList().ForEach((exceptionData) =>
                     {
        ...
        private IEnumerable<ExceptionEntity> ReadFromRestService()
        {
            try
            {

                var channelFactory = new WebChannelFactory<IGetExceptionsService>();

                //allways validate certificate.
                ServicePointManager.ServerCertificateValidationCallback = new RemoteCertificateValidationCallback(OnValidate);
                ...
                channelFactory.Credentials.UserName.UserName = (!String.IsNullOrEmpty(_settings.Domain) ? _settings.Domain + "\\" : String.Empty) + _settings.Username;
                channelFactory.Credentials.UserName.Password = _settings.Password;

                var channel = channelFactory.CreateChannel();

                return channel.GetExceptions();
            }
